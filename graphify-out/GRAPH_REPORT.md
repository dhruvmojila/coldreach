...
## Corpus Check
- 89 files · ~70,205 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1451 nodes · 5233 edges · 51 communities detected
- Extraction: 28% EXTRACTED · 72% INFERRED · 0% AMBIGUOUS · INFERRED: 3768 edges (avg confidence: 0.56)
...
1. `SourceResult` - 296 edges
2. `BaseSource` - 285 edges
3. `EmailSource` - 233 edges
4. `DomainResult` - 213 edges
5. `CacheStore` - 180 edges
6. `PipelineResult` - 173 edges
7. `SpiderFootSource` - 166 edges
8. `HarvesterSource` - 164 edges
9. `FinderConfig` - 156 edges
10. `GitHubSource` - 148 edges
...
Cohesion: 0.03
Nodes (133): _drain_job_queue(), find(), find_stream(), _find_with_progress(), _finder_config(), _finder_config_v2(), FindRequest, _resolve() (+125 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (165): ABC, BaseSource, fetch(), Abstract base class for all ColdReach email discovery sources.  Every source fol, Safe wrapper around :meth:`fetch` — never raises.          Returns the results l, A single email address found by a source.      Attributes     ----------     ema, Execution summary for one source run., Abstract base for all email discovery sources.      Subclasses must implement :m (+157 more)

### Community 2 - "Community 2"
Cohesion: 0.11
Nodes (162): _build_sources(), ColdReach local API server.  Exposes a FastAPI application on localhost:8765 so, Parameters for single-email verification., Parameters for a domain email discovery run., Parameters for single-email verification., Parameters for a domain email discovery run., Build FinderConfig from an API request., Build FinderConfig from an API request.      full_scan=True overrides quick and (+154 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (82): cache_clear(), cache_list(), Store *result* for *domain* in all available cache layers., Delete cached entries.          Parameters         ----------         domain:, Return all cached domains as (domain, cached_at, is_expired) tuples., Return basic cache statistics., Return a cached DomainResult for *domain*, or ``None`` on miss/expiry., cache_list() (+74 more)

### Community 4 - "Community 4"
Cohesion: 0.04
Nodes (52): check_holehe(), Holehe platform-presence check.  Uses the holehe library (github.com/megadose/ho, Check if *email* is registered on public platforms via holehe.      Parameters, mx_records(), normalized_email(), run_basic_pipeline(), check_reacher(), _parse_reacher_response() (+44 more)
...
Cohesion: 0.05
Nodes (30): verify(), BaseSettings, _banner(), cache(), cache_clear(), _configure_logging(), _domain_result_to_dict(), find() (+22 more)
...
Cohesion: 0.11
Nodes (10): _extract_domain_emails(), _strip_html(), _build_queries(), _extract_domain_emails(), _query_brave(), _query_ddg_lite(), _query_searxng(), TestExtractDomainEmails (+2 more)
...
Cohesion: 0.11
Nodes (18): _api(), Unit tests for coldreach.sources.spiderfoot  docker exec / subprocess calls are, Accepts both 'EMAILADDR' and 'Email Address' type strings., Emails like 'user@acme.com [apollo.io]' should be cleaned., _rows(), test_confidence_hint_is_positive(), test_fetch_results_returns_rows(), test_is_available_false_on_connect_error() (+10 more)
...
## Knowledge Gaps
- **115 isolated node(s):** `Test v2 scan API and poll for results.`, `ColdReach configuration via pydantic-settings.  All settings are read from envir`, `All runtime configuration for ColdReach.      Every field can be overridden by a`, `True if a Groq API key is configured.`, `True if the database backend is SQLite (no Docker needed).` (+110 more)
...
- **Why does `EmailSource` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 6`, `Community 10`, `Community 11`, `Community 13`, `Community 16`, `Community 19`, `Community 21`?**
  _High betweenness centrality (0.136) - this node is a cross-community bridge._
- **Why does `SourceResult` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 5`, `Community 11`, `Community 16`, `Community 19`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `CacheStore` connect `Community 2` to `Community 0`, `Community 3`, `Community 5`, `Community 7`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Are the 294 inferred relationships involving `SourceResult` (e.g. with `ScanJob` and `FindRequest`) actually correct?**
  _`SourceResult` has 294 INFERRED edges - model-reasoned connections that need verification._
- **Are the 280 inferred relationships involving `BaseSource` (e.g. with `ScanJob` and `FindRequest`) actually correct?**
  _`BaseSource` has 280 INFERRED edges - model-reasoned connections that need verification._
